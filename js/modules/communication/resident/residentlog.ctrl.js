app.controller('residentLogCtrl', function($scope, $timeout, $rootScope, $filter, $http, DTOptionsBuilder, DTColumnDefBuilder, LogService) {
  ...
  //on tab show
  // $('a[href="#algemeen"][data-toggle="tab"]').on('shown.bs.tab', function(e) {
  //   if (!$scope.loaded) {
  //     $scope.init($scope.formData.id);
  //     $scope.loaded = true;
  //   }
  // });
  ...
  $scope.init = function(id) {
    vm.residentId = id;
    $scope.getLogsOfResident(vm.residentId, 1, vm.pageCount);
  };
  ...
  $rootScope.$on('FILE_UPDATED', function(event, response) {
    ...
    LogService.getLogFromResident(vm.residentId, $rootScope.upload.id).then(function(data) {
  ...
  $scope.paginateTo = function(page) {
    $scope.isLoaded = false;
    $scope.getLogsOfResident(vm.residentId, page, vm.pageCount)
  };

  //API - Get all logs of resident
  $scope.getLogsOfResident = function(id, page, count) {
    $http.get("/api/residents/" + id + "/logs?page=" + page + "&count=" + count, {
        cache: false
      })
  ...
  $scope.addLogDraft = function($event) {
    ...
    if ($scope.formData.id) {
      ...
      if (!$rootScope.docDraft.id) {
        $http.post("/api/residents/" + $scope.formData.id + "/logs", {
            description: '.',
            log_type: 'none'
          })
          .then(function(response) {
    ...
    if (isValid) {
      if ($scope.formData.id) {
        ...
        if (!$rootScope.docDraft.id) {
          ...
          $http.post("/api/residents/" + $scope.formData.id + "/logs", {
            ...
            .then(function(response) {
              ...
              $scope.log2Add.log_type = preselectedLogType;
              $scope.ResidentLogForm.$setPristine();
              $scope.ResidentLogForm.$setUntouched();
        ...
        //Existing log (already documents added)
          ...
          $http.put("/api/residents/" + $scope.formData.id + "/logs/" + $rootScope.docDraft.id, {
              description: $scope.log2Add.description,
              log_type: $scope.log2Add.log_type.id,
              log_timestamp: dateFormattedToPost,
              is_draft: false
  ...
  $scope.deleteLog = function(index, id) {
    ...
    bootbox.confirm({
      ...
      callback: function(result) {
        //delete
        if (result) {
          $http.delete("/api/residents/" + $scope.formData.id + "/logs/" + id)
            .then(function(response) {
              //employee removed
  ...
  $scope.updateLog = function(idx, $itemid) {
    ...
    $http.put("/api/residents/" + $scope.formData.id + "/logs/" + $itemid, {
  ...
  $scope.deleteDocFromLog = function($event, logId, logOrder, docId) {
    ...
    bootbox.confirm({
      ...
      callback: function(result) {
        if (result) {
          //type: issues, issuedialogs, residents, decisions
